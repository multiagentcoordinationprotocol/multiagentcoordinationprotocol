# Invalid task-rules fixtures (negative schema tests)

Each file here is a Task Mode governance `rules` object that MUST be **rejected**
by `schemas/json/policy/task-rules.schema.json`. They are regression tests: if a
...
`task-rules.schema.json` carries only enum, type, and `minimum` constraints — no
...
nothing in the repository had ever validated a Task Mode `rules` object against
