...
> **Changelog — 1.6.0-draft:** §4.1 gains the **vacuous participation floor** rule. At `schema_version ≥ 3`, `commitment.require_vote_quorum: true` over an effective floor of zero — `voting.quorum` absent, or `value: 0` under either type — is now *defined* as equivalent to `require_vote_quorum: false`, and a runtime MUST evaluate it that way rather than rejecting the descriptor (issue #122). No schema keyword changes; the annotations in `decision-rules.schema.json` are updated, but nothing that validates. The combination was reported as a fourth instance of the fail-open family closed by issue #98; on analysis it is **not** one at `schema_version ≥ 3`. Every member of that family changed an **outcome** relative to a legal alternative spelling — `threshold: 0` made an all-`REJECT` round return Passed — whereas this one changes none: with a zero floor the quorum gate is trivially satisfied in every reachable state, and the `schema_version ≥ 3` empty-tally protection is supplied by the declared version itself rather than by the flag, so `true` over a zero floor and `false` are observationally identical. What was missing was therefore not a constraint but a statement of **meaning**: the equivalence was derivable and written down nowhere, which left an implementer free to reject the combination at admission. That is now forbidden. No schema keyword enforces the rule, and none can: `schema_version` is a field of the descriptor while the rule schemas validate a bare `rules` object, so a rules-schema arm cannot see the version it would need to discriminate on. The `schema_version ≤ 2` double duty is preserved verbatim — there the flag also decides whether an empty tally blocks a positive commitment, a zero floor does not disable that, and the legacy rule in §4.1 continues to REQUIRE the flag of any legacy policy intending a binding algorithm. `examples/discovery/policy_descriptor_decline.json` is exactly such a policy and is unaffected.
>
> **Changelog — 1.5.0-draft:** `description` becomes a **required** PolicyDescriptor field in `schemas/json/macp-policy-descriptor.schema.json`. §3 has listed five required fields including `description` since this RFC was written, and `docs/policy.md` says the same; the schema required only four, so a descriptor carrying no human-readable account of what its governance rules are for validated clean (issue #120). The RFC was right and the schema was under-constraining, so the schema moves rather than the prose. Nothing on disk is affected: all 26 descriptor instances in this repository already carry the field, and every downstream implementation already treats it as mandatory — both SDKs take it as a required positional builder argument. This is an admission-time tightening with no `schema_version` bump. Note the citation differs from the earlier entries in this series: §7 and §8 item 4 are worded for the *rule* JSON Schemas, and this is the first change to the **descriptor** schema, so neither clause literally covers it. The conclusion holds a fortiori rather than by citation — nothing in this specification directs a runtime to revalidate a stored descriptor against the descriptor schema at any point, so there is no revalidation path for a tightening to disturb. It is narrower than the 1.4.0-draft tightening, which changed admission for otherwise well-formed documents; here the rejected shape is one that no conformant producer emits. Presence only: no `minLength` is imposed, because `description` is human-readable text that affects no evaluation, and §8's policy-equality rule (`policy_id` + `schema_version` + `rules`) already excludes it.
>
> **Changelog — 1.4.0-draft:** rule objects are **closed**. Every object level in every standard mode's rule schema now sets `additionalProperties: false`, so an unknown key is rejected at admission instead of being silently ignored. The motivating case is a misspelling that leaves a governance parameter at its default: `objection_handling.veto_threshhold: 3` validated clean, `veto_threshold` stayed at its default of `1`, and a single blocking objection vetoed a commitment under a policy whose author had required three (issue #114). Nothing reported an error, because from the schema's point of view nothing was wrong. §4 gains a reserved **annotation namespace**: keys matching `^[_$]` are legal at every nesting level, carry no semantics, and MUST be ignored by evaluators. That escape is not cosmetic — without it this change would reject conformant documents already on disk, including the `$comment` in `examples/policy-registration-exchange.json` and the `_invalid_because` annotation on every negative fixture in `schemas/json/tests/`, the latter of which would have kept reporting "correctly rejected" while testing nothing. One exception is deliberate: `voting.weights` in `decision-rules.schema.json` is **not** closed, because its `additionalProperties` is a value schema typing the map's entries rather than an object-closing keyword, and its keys are participant identifiers. This is an admission-time tightening under §7 and §8 item 4 — no stored descriptor is revalidated, no `schema_version` bump — but unlike the earlier tightenings in this series it changes admission behavior for documents that are otherwise well-formed, so it is the one tightening in this series whose notification to downstream policy authors is tracked as its own follow-up rather than folded into the release note.
>
...
Rule objects are **closed**. Every object level in every standard mode's rule schema rejects properties it does not define — with one exception, `voting.weights`, set out below — and a runtime MUST reject at admission a `PolicyDescriptor` whose `rules` object carries an undefined key at any nesting level. Silently ignoring an unrecognized key is the more dangerous behavior: a misspelled parameter leaves the governance value the author intended at its default, so the session is governed by a policy nobody wrote and nothing reports an error.

Keys matching the pattern `^[_$]` — that is, beginning with an underscore or a dollar sign — are a **reserved annotation namespace**. They are legal at every closed object level of a `rules` object, they carry no governance semantics, and an evaluator MUST ignore them when computing any outcome. They exist so that documentation, provenance and tooling metadata can travel with a policy without widening its governance surface.

The namespace does **not** extend into maps whose keys are data rather than parameter names. `voting.weights` (§4.1) is the only such map defined by this document: its keys are participant identifiers and its `additionalProperties` types the map's values, so it is not a closed object level and an `_`-prefixed key there would name a participant, not an annotation. Do not place annotations inside it.

Because annotation keys are semantically inert, two `rules` objects differing only in them are equivalent for every purpose in this document: they resolve every parameter identically, so they satisfy the semantic-equality requirement of §2.2, and they are `rules`-equal for the replay and policy-equality comparison of §8.

One consequence is worth stating plainly, because the escape cannot avoid it: prefixing the name of a *real* parameter object with `_` — `_voting` rather than `voting` — produces a document that validates and is then ignored, which is the very failure closing these objects prevents. The namespace is for metadata that was never meant to be evaluated. A runtime MAY warn when an annotation key differs from a declared property name only by its prefix.
...
- `supermajority` — at least `threshold` of the decisive votes approve; the schema constrains `threshold` to be greater than `0.5` and **requires** it under this algorithm. Requiring it is not redundant with the floor: the field's documented default is `0.5`, a value this very algorithm forbids, so a `supermajority` block that omitted `threshold` would resolve to a bare majority wearing the name. The `majority` arm is deliberately asymmetric — it constrains the value without requiring the key, because `0.5` is legal there
...
**Denominator.** For the ratio-based algorithms (`majority`, `supermajority`, `weighted`) the denominator is the **decisive** votes — those cast as approve or reject. Abstentions are excluded and neither help nor hinder the ratio ([RFC-MACP-0007](RFC-MACP-0007-decision-mode.md) §4, which defines the `ABSTAIN` vote value and states that with no policy bound abstentions count toward no threshold). Under `weighted`, only weight-bearing votes are decisive (see the electorate rule above); a weight-`0` ballot joins the abstentions outside the ratio.
...
**Vacuous participation floor (schema_version ≥ 3).** Under `schema_version` 3 or later, the only effect of `commitment.require_vote_quorum` is the `voting.quorum` participation floor (see **`voting.quorum` is inert on its own**, above). When that floor is effectively zero — `voting.quorum` absent, or `value: 0` under either the `count` or the `percentage` type, which are the same declaration — the flag gates nothing: `require_vote_quorum: true` is equivalent to `require_vote_quorum: false`, and a runtime MUST evaluate it as such. The combination is an authoring smell, not an error: a runtime MUST NOT reject the descriptor at admission for it and MUST NOT substitute a floor the policy did not declare, though tooling MAY warn. This equivalence is scoped to `schema_version ≥ 3`. Under `schema_version` 1 and 2 it does NOT hold: the legacy empty-tally rule below gives the flag a second, independent role that a zero floor does not disable, and that rule requires the flag of any legacy policy intending a binding algorithm.
...
The **decline guard** for a vote-authorized negative commitment (≥1 **decisive** explicit `Vote` with `vote == "REJECT"`; optional `commitment.require_vote_quorum`) is defined with the Decision Mode terminal semantics — see [RFC-MACP-0007](RFC-MACP-0007-decision-mode.md) §6.2. For an **objection-authorized** decline the guard is waived whole, the `require_vote_quorum` conjunct and the `evaluation.*` prerequisites with it: that decline derives its authority from the recorded critical `Objection` rather than from the voting result, so a gate on the voting result has nothing to legitimize there. Both parameters are additive with conservative defaults that preserve pre-existing behavior; policies that use them declare `schema_version: 2` or later.
...
`registered_at_unix_ms` is runtime metadata and is not part of the policy's semantic identity. Replay and policy equality comparisons MUST use `policy_id` + `schema_version` + `rules` equality, not full descriptor byte comparison. `rules` equality is taken modulo the `^[_$]` annotation namespace of §4: keys in that namespace carry no semantics, so two `rules` objects differing only in them are equal for this purpose.
